#!/usr/bin/env node
// Lighthouse performance audit runner for respawn.kz.
// Usage: node scripts/audit-perf.mjs [baseUrl]
//   defaults to http://localhost:4321
// Prints a markdown table of {url, score} and writes JSON to docs/audit/perf-latest.json
...
try {
  ...
  for (const path of PATHS) {
    ...
    try {
      ...
      results.push(r);
      console.log(`${r.score >= 90 ? '✅' : '⚠️ '} ${path} — ${r.score}`);
    } catch (e) {
  ...
  writeFileSync(join(outDir, 'perf-latest.json'), JSON.stringify(results, null, 2));

  const avg = Math.round(results.reduce((s, r) => s + r.score, 0) / results.length);
  const passing = results.filter((r) => r.score >= 90).length;
  console.log(`\nSummary: ${passing}/${results.length} ≥90, avg ${avg}`);
  console.log(`Full report: docs/audit/perf-latest.json`);
} finally {
  await chrome?.kill();
}
